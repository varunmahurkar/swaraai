"""
FastAPI application for the SwaraAI backend.

Handles voice cloning, generation history, and model management.
"""
...
async def root():
    """Root endpoint."""
    return {"message": "SwaraAI API", "version": __version__}
...
async def export_profile(
    ...
    try:
        ...
        if not safe_name:
            safe_name = "profile"
        filename = f"profile-{safe_name}.swaraai.zip"
...
async def export_generation(
    ...
    try:
        ...
        filename = f"generation-{safe_text}.swaraai.zip"
...
async def transcribe_audio(
    ...
    Returns:
        ...
        - 202: Model is downloading (client should retry in 5-10 seconds)
        - 500: Error
    """
    # Save uploaded file to temporary location
    with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tmp:
    ...
    try:
        ...
        if whisper_model.is_loaded():
            print(f"[STT] Model {model_size} already loaded, using cached version")
        else:
            # Model not loaded - check if it's cached on disk
            print(f"[STT] Checking if model {model_size} is cached...")
            ...
            try:
                ...
                # Faster-Whisper models are under different naming
                # Check multiple possible cache locations
                possible_cache_paths = [
                    cache_dir / f"models--openai--whisper-{model_size}",
                    cache_dir / f"models--Systran--faster-whisper-{model_size}",
                ]

                is_cached = any(p.exists() for p in possible_cache_paths)

                if not is_cached:
                    print(f"[STT] Model {model_size} not cached, starting download...")
                    ...
                    async def download_whisper_background():
                        try:
                            print(f"[STT] Downloading model {model_size} in background...")
                            await whisper_model.load_model_async(model_size)
                            print(f"[STT] Download complete for {model_size}")
                        except Exception as e:
                ...
                # Re-raise HTTPException (202 Accepted)
                raise
            except Exception as e:
                print(f"[STT] Error checking cache: {e}")
                # Continue anyway - will attempt to load

        # Model is either cached or was just downloaded, transcribe now
        print(f"[STT] Starting transcription with model {model_size}")
        ...
        # Re-raise HTTP exceptions (like 202)
        raise
    except Exception as e:
...
async def get_model_status():
    ...
    for config in model_configs:
        try:
            ...
            # Build local_path:
            #   downloaded  → snapshots/<revision>/ (exact weights folder)
            #   downloading → models--org--name/    (cache root, shown during progress)
            #   not yet     → models--org--name/    (expected destination)
            local_path = None
            try:
                repo_cache = Path(hf_constants.HF_HUB_CACHE) / ("models--" + config["hf_repo_id"].replace("/", "--"))
                if downloaded:
                    snapshots_dir = repo_cache / "snapshots"
                    if snapshots_dir.exists():
                    ...
                    # Show where the model WILL be / IS being downloaded
...
async def trigger_model_download(request: models.ModelDownloadRequest):
    ...
    cache_dir = Path(hf_constants.HF_HUB_CACHE)
    blobs_dir = cache_dir / ("models--" + hf_repo_id.replace("/", "--")) / "blobs"
    cache_display_path = str(cache_dir / ("models--" + hf_repo_id.replace("/", "--")))

    async def download_in_background():
        """Download model in background, with a polling fallback for progress."""
        stop_polling = asyncio.Event()

        async def poll_cache_bytes():
            """Every 2 s, read bytes on-disk from the HF blob cache.
            This acts as a fallback when tqdm patching doesn't fire."""
            while not stop_polling.is_set():
                ...
                try:
                    if blobs_dir.exists():
                        ...
                        if on_disk > 0:
                            prog = progress_manager.get_progress(request.model_name)
                            # Only use fallback when tqdm hasn't given us a known total
                            if prog and prog.get("total", 0) == 0 and prog.get("status") == "downloading":
                                progress_manager.update_progress(
                                    model_name=request.model_name,
                                    current=on_disk,
                                    total=0,
                                    filename=cache_display_path,
                                    status="downloading",
                                )
                except Exception:
                    pass

        poll_task = asyncio.create_task(poll_cache_bytes())
        try:
    ...
    # Seed initial progress (path shown immediately, before any tqdm fires)
    progress_manager.update_progress(
        model_name=request.model_name,
        current=0,
        total=0,
...
async def cancel_model_download(request: models.ModelDownloadRequest):
    """
    Cancel an active model download and remove partial files.

    Since downloads run in threads we cannot forcibly stop them, but we:
    1. Remove the model from the task tracker so the 'downloading' flag clears immediately.
    2. Signal the SSE stream with a cancelled error so the frontend progress bar closes.
    3. Delete all .incomplete blob files so the next download starts fresh.
    """
    from huggingface_hub import constants as hf_constants
    ...
    hf_repo_id = hf_repo_map[model_name]

    # 1. Remove from task tracker (clears downloading flag on next status poll)
    task_manager.cancel_download(model_name)

    # 2. Signal SSE subscribers that the download was cancelled
    progress_manager.mark_error(model_name, "Cancelled by user")

    # 3. Delete .incomplete blobs so next download starts from scratch
    deleted_files = 0
    try:
        cache_dir = Path(hf_constants.HF_HUB_CACHE)
        blobs_dir = cache_dir / ("models--" + hf_repo_id.replace("/", "--")) / "blobs"
        ...
        print(f"[cancel] Warning: could not clean up partial files: {e}")

    print(f"[cancel] Cancelled {model_name}, removed {deleted_files} incomplete file(s)")
    return {"message": f"Download cancelled for {model_name}", "partial_files_removed": deleted_files}
...
async def shutdown_event():
    ...
    print("SwaraAI API shutting down...")
